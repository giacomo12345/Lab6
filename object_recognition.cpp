...
void myMatcher::findCorners() {
	...
	/*   better precision in case of books or other rectangular shapes
...
void  myMatcher::computeCenterAndDistance() {
	...
	for(int i = 0 ; i < obj.size() ; i++) {
		...
		points_obj.push_back(Point2f(obj[i].image.cols / 2, obj[i].image.rows / 2));
...
/* discar of outliers  */
void myMatcher::computeGoodScenePoints() {
	for (int i = 0; i < obj.size(); i++) {

		std::vector<Point2f> good_scene_points_temp;
		for (int j = 0; j < scene_points[i].size(); j++) {

			Point2f distance = center_points[i] - scene_points[i][j];
			float distance_module = sqrt(distance.x * distance.x + distance.y * distance.y);
			if (distance_module < max_distance[i]) good_scene_points_temp.push_back(scene_points[i][j]);
		}
...
/*  return good scene points*/
std::vector<std::vector<Point2f>> myMatcher::getGoodScenePoints() {
	return good_scene_points;
}